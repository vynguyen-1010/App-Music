...
import com.fhm.musicr.loader.medialoader.LastAddedLoader;
import com.fhm.musicr.loader.medialoader.TopAndRecentlyPlayedTracksLoader;
...
public class FeatureSongAdapter extends RecyclerView.Adapter<FeatureSongAdapter.ItemHolder>{
    ...
    public ArrayList<Song> mTopSongs = new ArrayList<>();
    public ArrayList<Song> mNewSongs = new ArrayList<>();
    ...
    public void setData(List<Song> data) {
        ...
        mTopSongs.clear();
        if(topsongs!=null) mTopSongs.addAll(topsongs);

        List<Song> newsongs = LastAddedLoader.getLastAddedSongs(mContext);// SongLoader.getSongsForCursor(TopTracksLoader.getCursor());
        mNewSongs.clear();
        if(newsongs!=null) mNewSongs.addAll(newsongs);
        initializeSong();
    }
    ...
    public void initializeSong() {
        ...
        Collections.shuffle(mTopSongs);
        Collections.shuffle(mNewSongs);

        int size = mNewSongs.size();
        for(int i = 0;i<2&&i<size;i++)
            mAllSongs.set(i, mNewSongs.get(i));


        size = mTopSongs.size();
        for(int i = 2;i<5&&i<size;i++)
            mAllSongs.set(i, mTopSongs.get(i-2));
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        void clickQuickPlayPause() {
            if(MusicPlayerRemote.getCurrentSong().id==mAllSongs.get(getAdapterPosition()).id) {
